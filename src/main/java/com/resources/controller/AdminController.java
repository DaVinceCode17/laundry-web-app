...
public class AdminController extends HttpServlet {
    ...
    // ============================================================
    // MOVE ORDER USING QUEUE NUMBER + CUSTOMER ID
    // ============================================================
    private void moveOrder(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        try {
            ...
            Order result = null;
            switch (action) {
                case "to_pending": result = service.moveToPending(queueNumber, customerId); break;
                case "to_wash": result = service.moveToWash(queueNumber, customerId); break;
                case "to_dry": result = service.moveToDry(queueNumber, customerId); break;
                case "to_iron": result = service.moveToIron(queueNumber, customerId); break;
                case "to_fold": result = service.moveToFold(queueNumber, customerId); break;
                case "for_pickup": result = service.moveToForPickup(queueNumber, customerId); break;
                case "to_deliver": result = service.moveToDeliver(queueNumber, customerId); break;
                case "claimed": result = service.moveToClaimed(queueNumber, customerId); break;
                case "claimed_from_pickup": result = service.moveToClaimedFromPickup(queueNumber, customerId); break;
                default: result = null;
            }
            
            if (result != null) {
                out.write("{\"success\":true,\"order\":{\"queueNumber\":" + result.getQueueNumber() + ",\"customerId\":" + result.getCustomerId() + ",\"status\":\"" + action + "\"}}");
            } else {
    ...
    private void updateSetPricingWeightAndPrice(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        try {
            ...
            boolean success = service.updateSetPricingWeightAndPrice(queueNumber, customerId, weight, price);
            if (success) {
                out.write("{\"success\":true,\"order\":{\"queueNumber\":" + queueNumber + ",\"customerId\":" + customerId + ",\"weight\":" + weight + ",\"price\":" + price + "}}");
            } else {
