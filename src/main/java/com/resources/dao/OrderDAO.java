...
public class OrderDAO {
    ...
    // ============================================================
    // MOVE METHODS USING QUEUE NUMBER + CUSTOMER ID
    // ============================================================
    
    public Order moveToPending(int queueNumber, int customerId) throws SQLException {
        return moveOrderByQueueAndCustomer(queueNumber, customerId, "set_pricing_orders", "pending_orders", "pending");
    }
    
    public Order moveToWash(int queueNumber, int customerId) throws SQLException {
        return moveOrderByQueueAndCustomer(queueNumber, customerId, "pending_orders", "to_wash_orders", "to_wash");
    }
    
    public Order moveToDry(int queueNumber, int customerId) throws SQLException {
        return moveOrderByQueueAndCustomer(queueNumber, customerId, "to_wash_orders", "to_dry_orders", "to_dry");
    }
    
    public Order moveToIron(int queueNumber, int customerId) throws SQLException {
        return moveOrderByQueueAndCustomer(queueNumber, customerId, "to_dry_orders", "to_iron_orders", "to_iron");
    }
    
    public Order moveToFold(int queueNumber, int customerId) throws SQLException {
        return moveOrderByQueueAndCustomer(queueNumber, customerId, "to_iron_orders", "to_fold_orders", "to_fold");
    }
    
    public Order moveToForPickup(int queueNumber, int customerId) throws SQLException {
        return moveOrderByQueueAndCustomer(queueNumber, customerId, "to_fold_orders", "for_pickup_orders", "for_pickup");
    }
    
    public Order moveToDeliver(int queueNumber, int customerId) throws SQLException {
        return moveOrderByQueueAndCustomer(queueNumber, customerId, "to_fold_orders", "to_deliver_orders", "to_deliver");
    }
    
    public Order moveToClaimed(int queueNumber, int customerId) throws SQLException {
        return moveOrderByQueueAndCustomer(queueNumber, customerId, "to_deliver_orders", "claimed_orders", "claimed");
    }
    
    public Order moveToClaimedFromPickup(int queueNumber, int customerId) throws SQLException {
        return moveOrderByQueueAndCustomer(queueNumber, customerId, "for_pickup_orders", "claimed_orders", "claimed");
    }
    
    // ============================================================
    // GENERIC MOVE ORDER USING QUEUE NUMBER + CUSTOMER ID
    // ============================================================
    private Order moveOrderByQueueAndCustomer(int queueNumber, int customerId, String fromTable, String toTable, String newStatus) throws SQLException {
        // SELECT - Hanapin ang order gamit ang queue_number + customer_id
        String selectSql = "SELECT o.*, c.first_name, c.last_name, c.address FROM " + fromTable + " o LEFT JOIN customers c ON o.customer_id = c.id WHERE o.queue_number = ? AND o.customer_id = ?";
        Order order = null;
        try (Connection conn = DatabaseConnection.getConnection();
        ...
        // DELETE mula sa lumang table gamit ang queue_number + customer_id
        String deleteSql = "DELETE FROM " + fromTable + " WHERE queue_number = ? AND customer_id = ?";
        try (Connection conn = DatabaseConnection.getConnection();
    ...
    // ============================================================
    // UPDATE WEIGHT & PRICE USING QUEUE NUMBER + CUSTOMER ID
    // ============================================================
    public boolean updateSetPricingWeightAndPrice(int queueNumber, int customerId, double weight, double price) throws SQLException {
        String sql = "UPDATE set_pricing_orders SET weight = ?, price = ? WHERE queue_number = ? AND customer_id = ?";
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            if (updated) {
                String logSql = "UPDATE laundry_logs SET weight = ?, price = ? WHERE queue_number = ? AND customer_id = ?";
